...
class HeavyChain:
    ...
    def build_weight(self) -> None:
        ...
        while stack:
            ...
            if u >= 0:
                ...
                for v in self.get_to_nodes(u):
                    if v != self.parent[u]:
                        self.cnt_son[u] += self.cnt_son[v]
                        if self.weight_son[u] == -1 or self.cnt_son[v] > self.cnt_son[self.weight_son[u]]:
                            self.weight_son[u] = v
        return

    def build_dfs(self) -> None:
        ...
        while stack:
            u, tp = stack.pop()
            self.dfn[u] = order
            self.rev_dfn[order] = u
            self.top[u] = tp
            order += 1
            # visit heavy children first, then visit light children
            w = self.weight_son[u]
            for v in self.get_to_nodes(u):
                if v != self.parent[u] and v != w:
                    stack.append((v, v))
            if w != -1:
                stack.append((w, tp))
        return

    def query_chain(self, u, v):
        # query the shortest path from u to v that passes through the chain segment
        pre = []
        post = []
        while self.top[u] != self.top[v]:
            if self.depth[self.top[u]] > self.depth[self.top[v]]:
                pre.append((self.dfn[u], self.dfn[self.top[u]]))
                u = self.parent[self.top[u]]
            else:
                post.append((self.dfn[self.top[v]], self.dfn[v]))
                v = self.parent[self.top[v]]

        a, b = self.dfn[u], self.dfn[v]
        pre.append((a, b))
        lca = a if a < b else b
        pre += post[::-1]
        return pre, lca
    ...
    def query_lca(self, u, v):
        # query the LCA nearest common ancestor of u and v
        while self.top[u] != self.top[v]:
            if self.depth[self.top[u]] < self.depth[self.top[v]]:
                u, v = v, u
            u = self.parent[self.top[u]]
